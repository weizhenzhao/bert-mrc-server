...
def find_fake_answer(sample):
    ...
    for doc in sample['documents']:
        ...
        max_related_score = 0
        for p_idx, para_tokens in enumerate(doc['segmented_paragraphs']):
            if 'segmented_answers' in sample.keys() and len(sample['segmented_answers']) > 0:
                related_score = metric_max_over_ground_truths(recall,
                                                              para_tokens,
    ...
    answer_tokens = set()
    if 'segmented_answers' in sample.keys():
        for segmented_answer in sample['segmented_answers']:
            answer_tokens = answer_tokens | set([token for token in segmented_answer])
    for d_idx, doc in enumerate(sample['documents']):
        if 'is_selected' in doc.keys() and not doc['is_selected']:
            continue
...
if __name__ == '__main__':
    examples = []
    with open(args.test_search_input_file, 'r', encoding='utf-8') as f:
        for line in tqdm(f.readlines()):
            sample = json.loads(line)
            find_fake_answer(sample)
            examples.append(sample)

    with open('../data/extracted/testset/search.test.json', 'w', encoding='utf8') as file2:
        for item in examples:
            file2.write(json.dumps(item, ensure_ascii=False))
            file2.write('\n')
