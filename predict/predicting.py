...
def find_best_indexes_for_passage(start_probs, end_probs, passage_len, question):
    ...
    if len(candidates) == 0:
        candidates.append({'start_idx': best_start_idxs[0][0], 'end_idx': best_end_idxs[0][0],
                           'score': best_start_idxs[0][1] + best_end_idxs[0][1]})
...
def find_multiple_answers(sample, start_probs, end_probs, prior_scores=(0.44, 0.23, 0.15, 0.09, 0.07)):
    ...
    for p_idx, passage in enumerate(sample['doc_tokens'][:args.max_para_num]):
        ...
        candidates = find_best_indexes_for_passage(start_probs[p_idx], end_probs[p_idx], passage_len,
    ...
    for candidate in all_candidates:
        ...
        for final_answer in final_answers:
            ...
            if candidate['p_idx'] == final_answer['p_idx']:
                if (c_start_idx >= a_start_idx and c_start_idx <= a_end_idx) or (
                        c_end_idx >= a_start_idx and c_end_idx <= a_end_idx):
                    duplicated = True
                    break
...
def find_best_answer(sample, start_probs, end_probs, prior_scores=(0.44, 0.23, 0.15, 0.09, 0.07)):
    ...
    for p_idx, passage in enumerate(sample['doc_tokens'][:args.max_para_num]):
        ...
        answer = "p" + sample['question_text'] + "。" + "".join(sample['doc_tokens'][p_idx])
    ...
    if best_p_idx is None or best_span is None:
        ...
        para = "p" + sample['question_text'] + "。" + "".join(sample['doc_tokens'][best_p_idx])
...
def evaluate(model, result_file):
    ...
    total_count = 0
    eval_examples = []
    with open('../dataset/predict.data', 'rb') as f:
        examples = f.readlines()
        for example in examples:
            eval_examples.append(json.loads(example))
        ...
        for step, example in enumerate(tqdm(eval_examples)):
            ...
            for p_num, doc_tokens in enumerate(example['doc_tokens'][:args.max_para_num]):
                (input_ids, input_mask, segment_ids) = \
                    predict_data.predict_data(question_text, doc_tokens, tokenizer,
            ...
            pred_answer = {'question_id': example['qas_id'],
            ...
            if 'answers' in example:
                ref_answers.append({'question_id': example['qas_id'],
                                    'question_type': example['question_type'],
                                    'question': example['question_text'],
                                    'answers': example['answers'],
                                    'predict_answers': [best_answer],
                                    'entity_answers': [[]],
                                    'yesno_answers': []})
...
def eval_all():
    ...
    output_config_file = "../bert_model/bert_config.json"
    config = BertConfig(output_config_file)
    model = BertForQuestionAnswering(config)
    model.load_state_dict(torch.load(output_model_file))
    model.to(torch.device("cuda", 0))
    evaluate(model, result_file="../metric/predicts.json")
