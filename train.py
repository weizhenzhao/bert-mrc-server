...
def train():
    ...
    for i in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(tqdm(train_dataloader, desc="Epoch")):
            ...
            input_ids, input_mask, segment_ids, start_positions, end_positions = \
                input_ids.to(device), input_mask.to(device), segment_ids.to(device), start_positions.to(
                    device), end_positions.to(device)

            # 计算loss
            loss, _, _ = model(input_ids, token_type_ids=segment_ids, attention_mask=input_mask,
