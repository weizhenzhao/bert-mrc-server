...
def read_squad_examples(zhidao_input_file, search_input_file, is_training=True):
    ...
    with open(search_input_file, 'r', encoding='utf-8') as f:
        for line in tqdm(f.readlines()):
            ...
            if 'answer_spans' in source.keys() and (len(source['answer_spans']) == 0):
                continue
            if 'answers' in source.keys() and source['answers'] == []:
                continue
            if 'match_scores' in source.keys() and (source['match_scores'][0] < 0.8):
                continue
            if 'answer_spans' in source.keys() and (source['answer_spans'][0][1] > args.max_seq_length):
                continue
            if is_training:
            ...
            if is_training:
                ...
                cut_fake_answer = "".join(new_doc_tokens[new_start_id:new_end_id])
                if source['fake_answers'][0] != cut_fake_answer:
                    continue
            else:
                new_end_id = new_start_id + 1
            ...
            if is_training:
                ...
                for document in source['documents']:
                    new_doc_tokens.append(document['paragraphs'][0])
    ...
    #             new_end_id = new_end_id - 1
    #             new_doc_tokens = []
    #             for document in source['document']:
    #                 new_doc_tokens.append(document['paragraphs'][0])
    #
    #             example = {
    #                 "qas_id": source['question_id'],
    ...
    #             }
    #             examples.append(example)
    with open("./predict.data", 'w', encoding="utf-8") as fout:
        for feature in examples:
            fout.write(json.dumps(feature, ensure_ascii=False) + '\n')
    print("len(features:", len(examples))
